...
for problem in selected_problems:
  ...
  for alg in algs.keys():
      alg_results[alg] = {}
      alg_data_x, alg_data_y = get_results('../QapSolver/results/' + problem + '_' + alg + '_' + str(size) + '.csv')

      alg_results[alg]['initial'] = list(map(lambda x: quality(x, optimal_score), alg_data_x))
      alg_results[alg]['final'] = list(map(lambda y: quality(y, optimal_score), alg_data_y))
